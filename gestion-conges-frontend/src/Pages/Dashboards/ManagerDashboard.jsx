  ...
  useEffect(() => {
    const fetchManagerData = async () => {
      try {
        ...
        const requestsArray = responseRequests.data.data || responseRequests.data;
      ...
      <div className="stats-grid">
        ...
        <div className="stat-card stat-card-success">
          ...
          <div className="stat-content">
            ...
            <span className="stat-value">{pendingRequests.length + monthAbsences}</span>
          </div>
          ...
          <div className="table-wrapper">
            <table className="modern-table">
              <thead>
                <tr>
                  <th>
                    <Users size={16} className="table-header-icon" /> Employé
                  </th>
                  <th>
                    <CheckCircle size={16} className="table-header-icon" /> Type
                  </th>
                  <th>
                    <Calendar size={16} className="table-header-icon" /> Dates
                  </th>
                  <th>
              ...
              <tbody>
                {pendingRequests.slice(0, 5).map((req, index) => (
                  <tr key={req.id} className="table-row-hover" style={{
                    ...
                    <td className="employee-cell">
                      <div className="employee-info">
                        ...
                        <span className="employee-name">{req.user?.name || "N/A"}</span>
                      </div>
                    </td>
                    <td>
                      <span className="badge-type">{req.leave_type?.name}</span>
                    </td>
                    ...
                    <td className="duration-cell">
                      <span className="duration-badge">{req.days_count || 0}j</span>
                    </td>
                  </tr>
                ))}
              </tbody>
